...
public record LeaderboardEntryResponse(String roomId, String roomName, int turnCount, long elapsedSeconds,
		int participantCount, String solverNickname, Instant endedAt) {

	public static LeaderboardEntryResponse from(GameResult result) {
		return new LeaderboardEntryResponse(result.getRoomId(), result.getRoomName(), result.getTurnCount(),
